...
문제 1. 모든 학생의 학생 번호, 이름, 휴대폰, 학과번호, 학과명을 조회하시오
SELECT `stdNo`, `stdName`, `stdHp`, `depNo`, `` FROM `Students`;
#학과명 : 학생 테이블에 없음. 학과번호-학과명 : `departments`
# SELECT `stdNo`, `stdName`, `stdHp`, b.`depNo`, `depName` FROM `Students` AS a JOIN `Departments` AS b On a.depNo = b.depNo;

문제 2. 모든 교수의 교수 번호, 이름, 휴대폰, 학과번호, 학과명을 조회하시오

#학과명 : 교수  테이블에 없음. 학과번호-학과명 : `departments`
#SELECT `proNo`, `proName`, `proHp`, b.`depNo`, `depName` FROM `Professors` AS a JOIN `Departments` AS b ON a.depNo = b.depNo;

문제3. 모든 강좌의 강좌번호, 강좌명, 담당교수명, 휴대폰을 조회하시오.

# 휴대폰 : 교수  테이블 
SELECT `lecNo`, `lecName`, `proName`, `proHp` FROM `Lectures` AS a JOIN `Professors` AS b ON a.proNo = b.proNo;

문제4. 모든 강좌의 강좌번호, 강좌명, 담당교수 번호, 담당교수명, 휴대폰, 학과 번호, 학과명을 조회하시오.

# 담당교수 번호 - 교수 테이블, 학과 테이블 - 학과 번호, 학과명 
#SELECT `lecNo`, `lecName`, b.`proNo`, `proName`, `proHP`, c.`depNo`, `depName` FROM `Lectures` 
#AS a JOIN `Professors` AS b ON a.proNo = b. proNo JOIN `Department` AS c ON b.depNo = c.depNo;

문제5. 모든 수강신청 내역에서 학생번호, 학생명, 강좌번호, 강좌명, 교수번호, 교수명을 조회하시오.

#학생명 - 학생 테이블, 강좌명 - 강좌 테이블, 교수명 - 교수 테이블
#SELECT * 
#FROM `Register` AS a 
...
문제 6. 수강신청 테이블에 출석점수, 중간고사 점수, 기말고사 점수를 임의로 입력하시오(1~100사이)
...
문제 7. 수강신청 테이블에 입력한 출석점수, 중간고사 점수, 기말고사 점수를 모두 합산 후 평균을 구해 총점에 입력하시오.

# 평균 :  AVG
#UPDATE `Register` SET `reTotal` = (`regAttenScore` + `regMidScore` + `regFianlScore`) / 3;

문제 8. 수강신청 테이블에 총점을 기준으로 A~F 등급을 입력하시오.
...
문제 9. 수강신청테이블에서 총점이 가장 큰 점수를 조회하시오.
SELECT `regTotal` FROM `Register` ORDER BY `regTotal` DESC LIMIT 1;

문제 10. 수강신청 테이블에서 정우성 학생의 총점의 평균을 조회하시오.
